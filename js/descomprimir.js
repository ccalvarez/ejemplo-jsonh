...
let compacto = fs.readFileSync('corte61escrutiniocomp.json', 'utf8');
...
Array.from(datos.e).forEach(e => {
  ...
  fs.writeFileSync(
    'corte61escrutiniodescomp' + e.id.toString() + '.json',
    JSON.stringify(e.l)
  );
});
...
fs.writeFileSync('corte61escrutiniodescomp.json', JSON.stringify(datos));
